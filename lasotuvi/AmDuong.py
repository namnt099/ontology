...
def ngayThangNam(nn, tt, nnnn, duongLich, timeZone=7):
    thangNhuan = 0
    ...
    if nn > 0 and \
       nn < 32 and tt < 13 and tt > 0:
        if duongLich is True:
            ...
            [nn, tt, nnnn] = L2S(nn, tt, nnnn, thangNhuan)
...
def namXemhan(nnxh):
    canNamXH = (nnxh+6) % 10 +1
    chiNamXH = (nnxh+8) % 12 + 1
    return [canNamXH, chiNamXH]
...
def timLLocTon(canNam):
    if thienCan[canNam]['vitriDiaBan'] == 5:
        return 6
    elif thienCan[canNam]['vitriDiaBan'] == 6:
        return 7
    elif thienCan[canNam]['vitriDiaBan'] == 7:
        return 6
    elif thienCan[canNam]['vitriDiaBan'] == 8:
        return 7
    elif thienCan[canNam]['vitriDiaBan'] in [11, 1]:
        return 12
    elif thienCan[canNam]['vitriDiaBan'] in [12, 2]:
        return 1
    elif thienCan[canNam]['vitriDiaBan'] == 3:
        return 3
    elif thienCan[canNam]['vitriDiaBan'] == 4:
        return 4
    elif thienCan[canNam]['vitriDiaBan'] == 9:
        return 9
    elif thienCan[canNam]['vitriDiaBan'] == 10:
        return 10
    else:
        raise Exception("Không tìm được L.Lộc Tồn")

def timLTrangSinh(canNam):
    if thienCan[canNam]['tenCan'] in ["Kỷ", "Mậu", "Nhâm", "Quý"]:
        return 9
    elif thienCan[canNam]['tenCan'] in ["Giáp", "Ất"]:
        return 12
    elif thienCan[canNam]['tenCan'] in ["Canh", "Tân"]:
        return 6
    elif thienCan[canNam]['tenCan'] in ["Bính", "Đinh"]:
        return 3
    else:
        return Exception("Không tìm được L.Sinh")

def amDuongNamXH(canNam):
    if thienCan[canNam]['tenCan'] in ["Ất", "Đinh", "Kỷ", "Tân", "Quý"]:
        return -1
    else:
        return 1
